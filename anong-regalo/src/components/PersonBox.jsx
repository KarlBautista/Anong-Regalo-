...
const PersonBox = ({ background, title, url }) => {
    const { setPerson, lang } = useUser();
    const navigate = useNavigate();

    const isOther = String(title).trim().toLowerCase() === 'other'
    ...
    const handleConfirm = () => {
      ...
      if (!cleaned) return
      setPerson(cleaned, background)
      setIsOpen(false)
      navigate(`/details/${encodeURIComponent(cleaned)}`)
    }
      ...
      {isOpen ? (
        ...
        >
          ...
          <div className='relative w-full max-w-md rounded-2xl bg-white border-2 border-[#D32F2F] p-5'>
            <h3 className='text-[#D32F2F] text-lg font-semibold mb-3'>{t(lang, 'other.modalTitlePerson')}</h3>
            <input
              autoFocus
              value={customValue}
